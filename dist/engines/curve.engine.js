...
exports.fetchBondingCurve = fetchBondingCurve;
exports.decodeBuySimulation = decodeBuySimulation;
exports.getCurvePhase = getCurvePhase;
exports.analyzeLaunch = analyzeLaunch;
...
const INITIAL_REAL_TOKEN_RESERVES_NEW = 800000000n * 10n ** BigInt(config_1.TOKEN_DECIMALS);
// ─── PDA helpers ─────────────────────────────────────────────────────────────
function getBondingCurvePda(mint) {
    const [pda] = web3_js_1.PublicKey.findProgramAddressSync([Buffer.from('bonding-curve'), mint.toBuffer()], config_1.PUMP_PROGRAM_ID);
    return pda;
}
...
function getAssociatedBondingCurve(mint, curveAddr) {
    const [ata] = web3_js_1.PublicKey.findProgramAddressSync([curveAddr.toBuffer(), config_1.TOKEN_PROGRAM_ID.toBuffer(), mint.toBuffer()], config_1.ASSOCIATED_TOKEN_PROGRAM_ID);
    return ata;
}
...
async function fetchBondingCurve(mint) {
    ...
    let account;
    try {
        account = await config_1.connection.getAccountInfo(curveAddress, 'confirmed');
    }
    catch (err) {
    ...
    if (account.owner.toBase58() !== config_1.PUMP_PROGRAM_ID.toBase58()) {
        console.warn(`[fetchBondingCurve] Account owner mismatch. Expected ${config_1.PUMP_PROGRAM_ID.toBase58()}, got ${account.owner.toBase58()}`);
        return null;
    }
    ...
    if (!actualDisc.equals(config_1.BONDING_CURVE_DISCRIMINATOR)) {
        console.warn(`[fetchBondingCurve] Discriminator mismatch.\n` +
            `  Expected: ${config_1.BONDING_CURVE_DISCRIMINATOR.toString('hex')}\n` +
    ...
    if (state.virtualTokenReserves === 0n || state.virtualSolReserves === 0n) {
        console.warn(`[fetchBondingCurve] Zero virtual reserves detected. ` +
...
function decodeBuySimulation(curve, solAmountLamports) {
    ...
    if (vSol === 0n || vToken === 0n) {
        return {
            solAmount: Number(solAmountLamports) / config_1.LAMPORTS_PER_SOL,
            tokensOut: '0',
            priceBefore: 0,
            priceAfter: 0,
            priceImpact: 0,
    ...
    const priceBefore = (Number(vSol) / config_1.LAMPORTS_PER_SOL) /
        (Number(vToken) / 10 ** config_1.TOKEN_DECIMALS);
    ...
    const priceAfter = newVToken > 0n
        ? (Number(newVSol) / config_1.LAMPORTS_PER_SOL) / (Number(newVToken) / 10 ** config_1.TOKEN_DECIMALS)
        : 0;
...
// ─── Phase ────────────────────────────────────────────────────────────────────
function getCurvePhase(progress) {
    if (progress < 33)
        return 'EARLY';
    if (progress < 66)
        return 'MID';
...
async function getPumpPriorityFee() {
    try {
        const fees = await config_1.connection.getRecentPrioritizationFees({
            lockedWritableAccounts: [config_1.PUMP_PROGRAM_ID],
        });
        ...
        const median = sorted.length % 2 === 0
            ...
            : sorted[mid];
        const congestionLevel = median > 100_000 ? 'HIGH' : median > 10_000 ? 'MEDIUM' : 'LOW';
...
async function getVelocity(curveAddr) {
    try {
        const sigs = await config_1.connection.getSignaturesForAddress(curveAddr, { limit: 1000 });
...
function computeProgress(realTokenReserves, complete) {
    if (complete)
        return 100; // graduated
    // Helper: compute progress for a given initial reserves value
    function tryCompute(initialReserves) {
        if (realTokenReserves >= initialReserves)
            return 0; // no tokens sold yet
        // Bigint-safe: multiply first to preserve precision, then divide
        const soldRatio = Number((initialReserves - realTokenReserves) * 1000000n / initialReserves) / 1_000_000;
        if (soldRatio < 0 || soldRatio > 1)
            return null; // invalid for this constant
        return soldRatio * 100;
    }
    // Try the config value first (793_100_000 * 10^6)
    const p1 = tryCompute(config_1.INITIAL_REAL_TOKEN_RESERVES);
    if (p1 !== null)
        return p1;
...
// ─── Main analysis ────────────────────────────────────────────────────────────
async function analyzeLaunch(mintStr) {
    // Validate mint address before doing anything
    let mint;
    try {
        mint = new web3_js_1.PublicKey(mintStr);
    }
    catch {
    ...
    const vSolFloat = Number(curveState.virtualSolReserves) / config_1.LAMPORTS_PER_SOL;
    const vTokenFloat = Number(curveState.virtualTokenReserves) / (10 ** config_1.TOKEN_DECIMALS);
    ...
    const totalSupplyHuman = Number(config_1.TOTAL_SUPPLY) / (10 ** config_1.TOKEN_DECIMALS); // = 1,000,000,000
    ...
    if (curveState.realSolReserves > 0n) {
        liquidityDepth = Number(curveState.realSolReserves) / config_1.LAMPORTS_PER_SOL;
    }
    else if (isGraduated) {
    ...
    // ── Buy simulations ──────────────────────────────────────────────────────────
    const simAmounts = [0.1, 0.5, 1.0].map(s => BigInt(Math.floor(s * config_1.LAMPORTS_PER_SOL)));
    ...
    const explosionRatio = progress > 0.5
    ...
    const [velocityData, congestionData, largestAccountsResult] = await Promise.allSettled([
        ...
        getPumpPriorityFee(),
        config_1.connection.getTokenLargestAccounts(mint, 'confirmed'),
    ]);
    const { recentTxCount, txPerHour } = velocityData.status === 'fulfilled'
        ? velocityData.value
        : { recentTxCount: 0, txPerHour: 0 };
    const { avgPriorityFee, congestionLevel } = congestionData.status === 'fulfilled'
        ? congestionData.value
        : { avgPriorityFee: 5000, congestionLevel: 'LOW' };
    const largestAccounts = largestAccountsResult.status === 'fulfilled'
